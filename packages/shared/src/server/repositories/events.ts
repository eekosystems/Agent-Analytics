...
export const getEventsForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      activetrace_observation_name: record.name,
      activetrace_trace_name: record.trace_name,
      activetrace_trace_id: record.trace_id,
      activetrace_url: `${baseUrl}/project/${projectId}/traces/${encodeURIComponent(record.trace_id as string)}?observation=${encodeURIComponent(record.id as string)}`,
      activetrace_user_url: record.user_id
        ? `${baseUrl}/project/${projectId}/users/${encodeURIComponent(record.user_id as string)}`
        : undefined,
      activetrace_id: record.id,
      activetrace_cost_usd: record.total_cost,
      activetrace_input_units: record.input_tokens,
      activetrace_output_units: record.output_tokens,
      activetrace_total_units: record.total_tokens,
      activetrace_session_id: record.session_id,
      activetrace_project_id: projectId,
      activetrace_project_name: projectName,
      activetrace_user_id: record.user_id || null,
      activetrace_latency: record.latency,
      activetrace_time_to_first_token: record.time_to_first_token,
      activetrace_release: record.release,
      activetrace_version: record.version,
      activetrace_model: record.provided_model_name,
      activetrace_level: record.level,
      activetrace_type: record.type,
      activetrace_tags: record.tags,
