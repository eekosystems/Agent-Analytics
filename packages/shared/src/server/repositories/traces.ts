...
export const getTracesForAnalyticsIntegrations = async function* (
  ...
  for await (const record of records) {
    yield {
      ...
      activetrace_trace_name: record.name,
      activetrace_url: `${baseUrl}/project/${projectId}/traces/${encodeURIComponent(record.id as string)}`,
      activetrace_user_url: record.user_id
        ? `${baseUrl}/project/${projectId}/users/${encodeURIComponent(record.user_id as string)}`
        : undefined,
      activetrace_cost_usd: record.total_cost,
      activetrace_count_observations: record.observation_count,
